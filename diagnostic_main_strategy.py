"""
Diagnostic for main strategy returns distribution
"""
import sys
...
def diagnose_main_strategy():
    ...
    # Generate same data as main.py demo
    ...
    # Generate price data (same logic as main.py)
    price_data = {}
    ...
    # Run backtest WITH risk management (like main.py)
    engine = BacktestEngine(
        prices=prices,
        initial_capital=1000000,
        commission_pct=0.0005,
        slippage_bps=3,
        enable_risk_management=True,  # KEY DIFFERENCE
    ...
    print("\n" + "="*80)
    print("MAIN STRATEGY RETURNS DISTRIBUTION DIAGNOSTIC")
    print("="*80)

    print(f"\nTotal periods: {len(returns)}")
    print(f"Non-zero returns: {(returns != 0).sum()} ({(returns != 0).sum() / len(returns) * 100:.1f}%)")
    ...
    print(f"\nReturn statistics (non-zero only):")
    print(f"Mean: {nonzero_returns.mean():.6f} ({nonzero_returns.mean() * 252:.2%} annualized)")
    print(f"Std: {nonzero_returns.std():.6f} ({nonzero_returns.std() * np.sqrt(252):.2%} annualized)")

    # Downside analysis
    negative_returns = returns[returns < 0]
    print(f"\nDownside returns statistics:")
    print(f"Count: {len(negative_returns)} ({len(negative_returns) / len(returns) * 100:.1f}%)")
    print(f"Mean: {negative_returns.mean():.6f}")
    print(f"Std: {negative_returns.std():.6f}")
    print(f"Annualized std: {negative_returns.std() * np.sqrt(252):.2%}")
    ...
    print(f"\nKey Metrics:")
    print(f"Volatility (annualized): {vol_annualized:.2%}")
    print(f"Downside deviation (annualized): {downside_dev_annualized:.2%}")
    ...
    downside_returns_excess = excess_returns[excess_returns < 0]
    downside_std_excess = downside_returns_excess.std()
    sortino = np.sqrt(252) * excess_returns.mean() / downside_std_excess
    sharpe = np.sqrt(252) * excess_returns.mean() / returns.std()

    print(f"\nSortino/Sharpe Analysis:")
    print(f"Sharpe: {sharpe:.2f}")
    print(f"Sortino: {sortino:.2f}")
    print(f"Sortino/Sharpe ratio: {sortino/sharpe:.2f}x")
